...
const BigCEditor: React.FC<EditorProps> = ({ data, setData, onPrint, onExportPDF, storeType = 'BIGC' }) => {
  ...
  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem('productCatalog_bigc');
    if (saved) {
      try {
  ...
  const handleSaveCatalog = (newCatalog: ProductTemplate[]) => {
    ...
    localStorage.setItem('productCatalog_bigc', JSON.stringify(newCatalog));
  };
  ...
  const generateRandomItems = () => {
    ...
    let newInvoiceNo = "";
    if (storeType === 'BIGC') {
      // New Logic: 660000 + CashierPrefix(3) + 000 + POSLast2(2) + 00 + TicketSuffix(6) + YYYYMMDD + HHMMSS
      const cashierId = (randomStaff.split('-')[0] || '103112').trim();
      const cashierPrefix = cashierId.substring(0, 3);
      const posSuffix = randomCounter.padStart(3, '0').substring(1, 3);
      const ticketSuffix = String(Math.floor(Math.random() * 1000000)).padStart(6, '0');

      const yyyy = now.getFullYear();
      const mm = String(now.getMonth() + 1).padStart(2, '0');
      const dd = String(now.getDate()).padStart(2, '0');
      const hh = String(randomHour).padStart(2, '0');
      const min = String(randomMin).padStart(2, '0');
      const ss = String(randomSec).padStart(2, '0');

      newInvoiceNo = `660000${cashierPrefix}000${posSuffix}00${ticketSuffix}${yyyy}${mm}${dd}${hh}${min}${ss}`;
    } else {
      const storeCode = '00305'; // Fallback for other store types if needed
      const counterStr = randomCounter.padStart(3, '0');
      let yStr = String(now.getFullYear()).slice(-2);
      let mStr = String(now.getMonth() + 1).padStart(2, '0');
      let dStr = String(now.getDate()).padStart(2, '0');
    ...
    <div className="editor-section">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <button className="btn btn-primary" style={{ backgroundColor: '#dc2626' }} onClick={onPrint}>
        ...
        <div className="config-container" style={{ marginBottom: '1.5rem', padding: '1.5rem', background: 'rgba(255,255,255,0.02)', borderRadius: '1rem', border: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr 1fr', gap: '1rem' }}>
            ...
            <div className="input-group">
              ...
              <input
                placeholder="6 số cuối"
                value={data.invoiceNo.length === 36 ? data.invoiceNo.substring(16, 22) : ""}
                onChange={(e) => {
                  const val = e.target.value.replace(/\D/g, '').slice(0, 6);
                  if (data.invoiceNo.length === 36) {
                    const prefix = data.invoiceNo.substring(0, 16);
                    const suffix = data.invoiceNo.substring(22);
                    setData(prev => ({ ...prev, invoiceNo: prefix + val.padStart(6, '0') + suffix }));
                  }
                }}
                style={{
                  fontWeight: 'bold',
                  color: '#dc2626',
                  border: (data.invoiceNo.length === 36 && data.invoiceNo.substring(16, 22).length !== 6) ? '1.5px solid #ef4444' : '1px solid var(--border)',
                  outline: 'none'
                }}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  let newInvoiceNo = "";
                  if (storeType === 'BIGC') {
                    const cashierId = (data.staffName.split('-')[0] || '103112').trim();
                    const cashierPrefix = cashierId.substring(0, 3);
                    const posSuffix = data.counterId.padStart(3, '0').substring(1, 3);
                    const ticketSuffix = String(Math.floor(Math.random() * 1000000)).padStart(6, '0');
          ...
          <div style={{ fontSize: '0.9rem', fontWeight: 'bold', marginBottom: '1rem', color: '#dc2626' }}>Cấu hình chữ trên Bill (Slogan, QR, Footer)</div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
            <div className="input-group">
              <label>Slogan Dòng 1</label>
              <input name="sloganLine1" value={data.sloganLine1 || ''} onChange={handleStoreChange} />
            </div>
            <div className="input-group">
              <label>Slogan Dòng 2</label>
              <input name="sloganLine2" value={data.sloganLine2 || ''} onChange={handleStoreChange} />
            </div>
            <div className="input-group">
              <label>Slogan Dòng 3</label>
              <input name="sloganLine3" value={data.sloganLine3 || ''} onChange={handleStoreChange} />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="input-group">
              ...
              <input name="qrText" value={data.qrText || ''} onChange={handleStoreChange} />
            </div>
            <div className="input-group">
              <label>Nhãn thuế (VAT Label)</label>
              <input name="taxLabel" value={data.taxLabel || ''} onChange={handleStoreChange} />
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="input-group">
              <label>Lời chào 1 (Footer)</label>
              <input name="footerLine1" value={data.footerLine1 || ''} onChange={handleStoreChange} />
            </div>
            <div className="input-group">
              <label>Lời chào 2 (Footer)</label>
              <input name="footerLine2" value={data.footerLine2 || ''} onChange={handleStoreChange} />
            </div>
          </div>

          <div className="input-group">
            <label>Thông tin pháp lý / Lưu ý chân trang</label>
            <textarea name="footerNote" value={data.footerNote || ''} onChange={handleStoreChange} rows={2} />
          </div>
            ...
            <div className="p-8 text-[#94a3b8] space-y-6">
              <div className="space-y-4">
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '2rem', height: '2rem', borderRadius: '50%', backgroundColor: '#dc2626', display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontWeight: 'bold' }}>1</div>
                  <div>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '2rem', height: '2rem', borderRadius: '50%', backgroundColor: '#dc2626', display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontWeight: 'bold' }}>2</div>
                  <div>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '2rem', height: '2rem', borderRadius: '50%', backgroundColor: '#dc2626', display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontWeight: 'bold' }}>3</div>
                  <div>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '2rem', height: '2rem', borderRadius: '50%', backgroundColor: '#dc2626', display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontWeight: 'bold' }}>4</div>
                  <div>
